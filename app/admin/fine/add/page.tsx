...
import { ChevronLeft, LogOut, Search, Check, Users, Receipt, Banknote, AlertCircle, Loader2, Sparkles } from "lucide-react"
  ...
  return (
    <div className="min-h-screen relative bg-[#f8fafc] overflow-x-hidden font-sans pb-32">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none z-0">
        <div className="absolute top-[-5%] left-[-10%] w-[500px] h-[500px] rounded-full bg-indigo-300/30 blur-[100px]" />
        <div className="absolute top-[20%] right-[-5%] w-[400px] h-[400px] rounded-full bg-purple-300/30 blur-[100px]" />
        <div className="absolute bottom-[-10%] left-[20%] w-[600px] h-[600px] rounded-full bg-blue-300/20 blur-[120px]" />
      </div>
            ...
            <div className="rounded-[24px] backdrop-blur-xl bg-white/60 border border-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-5 sm:p-6 flex flex-col">
              <div className="flex items-center justify-between mb-5">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-2xl bg-violet-100/80 flex items-center justify-center border border-violet-200/50">
                    <Users className="w-5 h-5 text-violet-600" />
                  </div>
                  <div>
                    <h2 className="text-lg font-bold text-gray-900 leading-tight">1. Select Students</h2>
                    <p className="text-sm text-gray-500 font-medium">Assign fine to individuals</p>
                  </div>
                </div>
                {selectedStudents.size > 0 && (
                  <div className="bg-indigo-600 text-white text-sm font-semibold px-3 py-1 rounded-full shadow-md shadow-indigo-200 border border-indigo-500 animate-in fade-in zoom-in">
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <div className="relative group flex-1">
                    ...
                    <Input
                      ...
                      className="pl-11 h-12 rounded-xl bg-white/40 border-white/60 focus:bg-white focus:ring-2 focus:ring-violet-100 shadow-sm transition-all text-base"
                    />
                  </div>
                  <Button
                    variant="outline"
                    ...
                    className="h-12 rounded-xl border-gray-200 bg-white/50 hover:bg-violet-50 hover:text-violet-700 hover:border-violet-200 shadow-sm shrink-0"
                  >
                      ...
                      <div className="max-h-[310px] overflow-y-auto pr-2 space-y-2 custom-scrollbar p-1">
                        {students.length === 0 && !loading ? (
                          <div className="text-center py-12 text-gray-500 flex flex-col items-center">
                            <Users className="w-10 h-10 mb-3 opacity-30 text-violet-400" />
                          ...
                          students.map((student) => (
                            <label
                              ...
                              className={`group relative flex items-center p-3 sm:p-4 cursor-pointer rounded-2xl transition-all duration-300 border ${selectedStudents.has(student.id)
                                ? "bg-white border-violet-200 shadow-[0_4px_20px_-4px_rgba(139,92,246,0.15)] ring-1 ring-violet-50 scale-[1.01] z-10"
                                : "bg-white/60 border-transparent hover:bg-white hover:border-gray-200 hover:shadow-sm"
                                }`}
                            >
                              <div className="flex-1 min-w-0 pr-4">
                                <p className={`text-base font-bold truncate transition-colors ${selectedStudents.has(student.id) ? 'text-violet-900' : 'text-gray-900 group-hover:text-violet-700'}`}>
                                  {student.name}
                                </p>
                                <div className="flex flex-wrap items-center gap-2 mt-1.5">
                                  ...
                                  {student.class_name && (
                                    <span className="inline-flex items-center rounded-md bg-indigo-50 px-2 py-0.5 text-xs font-semibold text-indigo-700 border border-indigo-100/50">
                                      {student.class_name}
                                    </span>
                                  )}
                                </div>
                              ...
                              <div className="ml-auto shrink-0 relative flex items-center">
                                <div className={`w-7 h-7 rounded-xl flex items-center justify-center transition-all duration-300 ${selectedStudents.has(student.id)
                                  ? "bg-gradient-to-br from-violet-500 to-indigo-500 shadow-md shadow-violet-200/50 border border-violet-400"
                                  : "bg-white border-2 border-gray-300 group-hover:border-violet-300"
                                  }`}>
          ...
          <div className="lg:col-span-5 flex flex-col gap-6">
            <div className="rounded-[24px] backdrop-blur-xl bg-white/60 border border-white isolate shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-5 sm:p-6 lg:sticky lg:top-28">
              <div className="flex items-center gap-3 mb-5">
                <div className="w-10 h-10 rounded-2xl bg-indigo-100/80 flex items-center justify-center border border-indigo-200/50">
                  <Receipt className="w-5 h-5 text-indigo-600" />
                </div>
                <div>
                  <h2 className="text-lg font-bold text-gray-900 leading-tight">2. Select Fine</h2>
                  <p className="text-sm text-gray-500 font-medium">Choose fine category</p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="relative group">
                  ...
                  <Input
                    ...
                    className="pl-11 h-12 rounded-xl bg-white/40 border-white/60 focus:bg-white focus:ring-2 focus:ring-indigo-100 shadow-sm transition-all text-base"
                  />
                </div>
                ...
                <div className="space-y-3 max-h-[320px] overflow-y-auto pr-2 custom-scrollbar">
                  {filteredFines.length === 0 ? (
                    <div className="text-center py-8 text-gray-500 flex flex-col items-center">
                      <AlertCircle className="w-8 h-8 mb-2 opacity-50 text-indigo-400" />
                      <p className="text-sm font-medium">No fines found matching criteria.</p>
                    </div>
                  ) : (
                    filteredFines.map((fine) => (
                      <button
                        ...
                        className={`w-full text-left p-4 rounded-2xl transition-all duration-300 border focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 relative overflow-hidden group ${selectedFine?.id === fine.id
                          ? "bg-white border-indigo-300 shadow-[0_8px_20px_-4px_rgba(99,102,241,0.15)] scale-[1.01]"
                          : "bg-white/40 border-transparent hover:bg-white/70 hover:border-indigo-100 hover:scale-[1.01]"
                          }`}
                      >
                        ...
                        <div className="flex justify-between items-start gap-4 pl-1">
                          <div className="flex-1">
                            <h3 className={`font-semibold text-base transition-colors ${selectedFine?.id === fine.id ? 'text-indigo-900' : 'text-gray-900 group-hover:text-indigo-800'}`}>
                              {fine.name}
                            </h3>
                            {fine.description && (
                          ...
                          <div className="shrink-0 bg-indigo-50/80 text-indigo-700 font-bold px-3 py-1.5 rounded-xl text-sm border border-indigo-100/50 shadow-sm">
                ...
                <div className={`transition-all duration-300 overflow-hidden ${selectedFine ? 'max-h-[200px] opacity-100 mt-4' : 'max-h-0 opacity-0'}`}>
                  <div className="pt-4 border-t border-gray-200/50 space-y-2">
                    ...
                    <textarea
                      ...
                      className="w-full px-4 py-3 bg-white/50 border border-white shadow-sm focus:bg-white focus:ring-2 focus:ring-indigo-100 rounded-xl text-sm outline-none transition-all placeholder:text-gray-400 min-h-[80px]"
                    />
                  </div>
                </div>
              </div>
      ...
      <div className={`fixed bottom-0 left-0 right-0 lg:bottom-6 lg:left-1/2 lg:-translate-x-1/2 lg:w-full lg:max-w-[70rem] z-50 transition-transform duration-500 ${selectedStudents.size > 0 && selectedFine ? 'translate-y-0' : 'translate-y-full lg:translate-y-[150%]'} pointer-events-none`}>
        <div className="w-full sm:w-auto mx-auto px-4 pb-4 lg:p-0 pointer-events-auto">
          <div className="backdrop-blur-2xl bg-white/85 border border-white/60 sm:rounded-3xl rounded-2xl p-4 lg:px-6 lg:py-5 shadow-[0_-10px_40px_-10px_rgba(0,0,0,0.1)] lg:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.15)] flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6 bg-gradient-to-r from-white/90 to-indigo-50/50">

            <div className="flex items-center gap-4 w-full sm:w-auto">
              <div className="h-12 w-12 sm:h-14 sm:w-14 rounded-2xl bg-gradient-to-br from-indigo-100 to-violet-100 flex items-center justify-center border border-indigo-200/50 shrink-0 shadow-inner">
                <Banknote className="w-6 h-6 sm:w-7 sm:h-7 text-indigo-600" />
              </div>
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <p className="text-xs sm:text-sm text-gray-500 font-bold uppercase tracking-wider">Total Fine</p>
                  <span className="inline-flex bg-indigo-100 text-indigo-700 text-[10px] font-bold px-2 py-0.5 rounded-full">
                ...
                <p className="text-2xl sm:text-3xl font-extrabold text-gray-900 leading-tight">
                  ₹{selectedFine ? (selectedFine.amount * selectedStudents.size).toFixed(2) : "0.00"}
                </p>
              </div>
            </div>
            ...
            <div className="flex gap-3 w-full sm:w-auto shrink-0">
              ...
              <Button
                ...
                className="w-full sm:w-auto rounded-2xl bg-gradient-to-r from-indigo-600 via-violet-600 to-purple-600 hover:from-indigo-700 hover:via-violet-700 hover:to-purple-700 text-white shadow-lg shadow-indigo-200 border border-indigo-500/50 h-12 sm:h-14 px-8 font-bold text-base sm:text-lg transition-all active:scale-95 group relative overflow-hidden"
              >
                ...
                <span className="relative flex items-center justify-center z-10 w-full">
                  {submitting ? (
                    ...
                    <><Sparkles className="w-5 h-5 sm:w-6 sm:h-6 mr-3" /> Issue Fine</>
                  )}
                </span>
              </Button>
            </div>

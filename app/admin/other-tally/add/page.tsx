...
import { ChevronLeft, LogOut, Search, Check, Users, AlertCircle, Loader2, Sparkles, PlusCircle } from "lucide-react"
...
export default function AddOtherTallyPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error("Failed to add other tally:", error)
      toast.error(error instanceof Error ? error.message : "Failed to add other tally")
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <div className="min-h-screen relative bg-[#f8fafc] overflow-x-hidden font-sans pb-32">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none z-0">
        ...
        <div className="absolute bottom-[-10%] left-[20%] w-[600px] h-[600px] rounded-full bg-red-300/20 blur-[120px]" />
      </div>
              ...
              <div className="flex items-center justify-between mb-5">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500 font-medium">Assign other tally to individuals</p>
                  </div>
                </div>
                {selectedStudents.size > 0 && (
                  <div className="bg-orange-500 text-white text-sm font-semibold px-3 py-1 rounded-full shadow-md shadow-orange-200 border border-orange-400 animate-in fade-in zoom-in">
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <div className="relative group flex-1">
                    ...
                    <Input
                      ...
                      className="pl-11 h-12 rounded-xl bg-white/40 border-white/60 focus:bg-white focus:ring-2 focus:ring-orange-100 shadow-sm transition-all text-base"
                    />
                  </div>
                  <Button
                    variant="outline"
                    ...
                    className="h-12 rounded-xl border-gray-200 bg-white/50 hover:bg-orange-50 hover:text-orange-700 hover:border-orange-200 shadow-sm shrink-0"
                  >
                      ...
                      <div className="max-h-[310px] overflow-y-auto pr-2 space-y-2 custom-scrollbar p-1">
                        {students.length === 0 && !studentLoading ? (
                          <div className="text-center py-12 text-gray-500 flex flex-col items-center">
                            <Users className="w-10 h-10 mb-3 opacity-30 text-orange-400" />
                          ...
                          students.map((student) => (
                            <label
                              ...
                              className={`group relative flex items-center p-3 sm:p-4 cursor-pointer rounded-2xl transition-all duration-300 border ${selectedStudents.has(student.id)
                                ? "bg-white border-orange-200 shadow-[0_4px_20px_-4px_rgba(249,115,22,0.15)] ring-1 ring-orange-50 scale-[1.01] z-10"
                                : "bg-white/60 border-transparent hover:bg-white hover:border-gray-200 hover:shadow-sm"
                                }`}
                            >
                              <div className="flex-1 min-w-0 pr-4">
                                <p className={`text-base font-bold truncate transition-colors ${selectedStudents.has(student.id) ? 'text-orange-900' : 'text-gray-900 group-hover:text-orange-700'}`}>
                                  {student.name}
                                </p>
                              ...
                              <div className="ml-auto shrink-0 relative flex items-center">
                                <div className={`w-7 h-7 rounded-xl flex items-center justify-center transition-all duration-300 ${selectedStudents.has(student.id)
                                  ? "bg-gradient-to-br from-orange-400 to-amber-500 shadow-md shadow-orange-200/50 border border-orange-400"
                                  : "bg-white border-2 border-gray-300 group-hover:border-orange-300"
                                  }`}>
          ...
          <div className="lg:col-span-5 flex flex-col gap-6">
            <div className="rounded-[24px] backdrop-blur-xl bg-white/60 border border-white isolate shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-5 sm:p-6 lg:sticky lg:top-28">
              <div className="flex items-center gap-3 mb-5">
                ...
                <div>
                  <h2 className="text-lg font-bold text-gray-900 leading-tight">2. Select Type</h2>
                  <p className="text-sm text-gray-500 font-medium">Choose other tally type</p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="relative group">
                  ...
                  <Input
                    placeholder="Search other tallies..."
                    value={tallySearch}
                ...
                <div className="space-y-3 max-h-[320px] overflow-y-auto pr-2 custom-scrollbar">
                  {loading ? (
                    ...
                    <div className="text-center py-8 text-gray-500 flex flex-col items-center">
                      <AlertCircle className="w-8 h-8 mb-2 opacity-50 text-amber-400" />
                      <p className="text-sm font-medium">No other tallies found.</p>
                    </div>
                  ) : (
                    filteredTallies.map((tally) => (
                      <button
                        ...
                        className={`w-full text-left p-4 rounded-2xl transition-all duration-300 border focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-500 relative overflow-hidden group ${selectedTally?.id === tally.id
                          ? "bg-white border-amber-300 shadow-[0_8px_20px_-4px_rgba(245,158,11,0.15)] scale-[1.01]"
                          : "bg-white/40 border-transparent hover:bg-white/70 hover:border-amber-100 hover:scale-[1.01]"
                          }`}
                      >
                        ...
                        <div className="flex justify-between items-start gap-4 pl-1">
                          <div className="flex-1">
                            <h3 className={`font-semibold text-base transition-colors ${selectedTally?.id === tally.id ? 'text-amber-900' : 'text-gray-900 group-hover:text-amber-800'}`}>
                              {tally.name}
                            </h3>
                            {tally.description && (
                          ...
                          <div className="shrink-0 bg-amber-50/80 text-amber-700 font-bold px-3 py-1.5 rounded-xl text-sm border border-amber-100/50 shadow-sm">
                ...
                <div className={`transition-all duration-300 overflow-hidden ${selectedTally ? 'max-h-[200px] opacity-100 mt-4' : 'max-h-0 opacity-0'}`}>
                  <div className="pt-4 border-t border-gray-200/50 space-y-2">
                    ...
                    <textarea
                      ...
                      className="w-full px-4 py-3 bg-white/50 border border-white shadow-sm focus:bg-white focus:ring-2 focus:ring-amber-100 rounded-xl text-sm outline-none transition-all placeholder:text-gray-400 min-h-[80px]"
                    />
                  </div>
                </div>
              </div>
          ...
          <div className="backdrop-blur-2xl bg-white/85 border border-white/60 sm:rounded-3xl rounded-2xl p-4 lg:px-6 lg:py-5 shadow-[0_-10px_40px_-10px_rgba(0,0,0,0.1)] lg:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.15)] flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6 bg-gradient-to-r from-white/90 to-orange-50/50">
            ...
            <div className="flex gap-3 w-full sm:w-auto shrink-0">
              ...
              <Button
                ...
                className="w-full sm:w-auto rounded-2xl bg-gradient-to-r from-orange-500 via-amber-500 to-yellow-500 hover:from-orange-600 hover:via-amber-600 hover:to-yellow-600 text-white shadow-lg shadow-orange-200 border border-orange-400/50 h-12 sm:h-14 px-8 font-bold text-base sm:text-lg transition-all active:scale-95 group relative overflow-hidden"
              >

...
import { ChevronLeft, LogOut, Plus, Search, Star, Loader2, ClipboardList, TrendingUp, Users, Trash2 } from "lucide-react"
...
export default function TallyManagementPage() {
  ...
  const fetchTallies = async () => {
    try {
      ...
      data.forEach((tally) => {
        if (tally.tallyType !== 'NORMAL') return
    ...
    <div className="min-h-screen relative bg-[#f8fafc] overflow-x-hidden font-sans pb-24">
          ...
          <div className="flex items-center justify-between gap-2 flex-wrap sm:flex-nowrap">
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
              <DownloadButton
                ...
                filename="tally-overview"
                title="Tally Overview Report"
              />
              <Link href="/admin/tally/manage" className="flex-1 sm:flex-none">
                <Button variant="outline" className="w-full sm:w-auto rounded-xl gap-1.5 bg-white/50 border-gray-200 hover:bg-blue-50 hover:text-blue-700 shadow-sm transition-colors text-sm px-3 sm:px-4">
                  <ClipboardList className="w-4 h-4" />
                  <span>Manage</span>
                </Button>
              </Link>
              <Link href="/admin/tally/add" className="flex-1 sm:flex-none">
                <Button className="w-full sm:w-auto rounded-xl gap-1.5 bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-700 hover:to-blue-700 text-white shadow-md shadow-blue-200 transition-all text-sm px-3 sm:px-4 border border-blue-500/50">
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-6">
        ...
        <div className="hidden sm:grid grid-cols-3 gap-4 lg:gap-6">
          <div className="rounded-2xl backdrop-blur-xl bg-white/60 border border-white shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] p-5 flex items-center gap-4">
            <div className="w-12 h-12 rounded-xl bg-cyan-100 flex items-center justify-center text-cyan-600 border border-cyan-200/50">
              <Users className="w-6 h-6" />
            </div>
            <div>
              <p className="text-sm font-medium text-gray-500">Total Filtered Students</p>
              <p className="text-2xl font-bold text-gray-900">{filteredTallies.length}</p>
            </div>
          </div>
          <div className="rounded-2xl backdrop-blur-xl bg-white/60 border border-white shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] p-5 flex items-center gap-4">
            <div className="w-12 h-12 rounded-xl bg-blue-100 flex items-center justify-center text-blue-600 border border-blue-200/50">
              <ClipboardList className="w-6 h-6" />
            </div>
            <div>
              <p className="text-sm font-medium text-gray-500">Total Tallies Listed</p>
              <p className="text-2xl font-bold text-gray-900">{totalFilteredTallies}</p>
            </div>
          </div>
          <div className="rounded-2xl backdrop-blur-xl bg-white/60 border border-white shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] p-5 flex items-center gap-4">
            <div className="w-12 h-12 rounded-xl bg-emerald-100 flex items-center justify-center text-emerald-600 border border-emerald-200/50">
              <TrendingUp className="w-6 h-6" />
            </div>
            <div>
              <p className="text-sm font-medium text-gray-500">Rupees Listed</p>
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-center">
            <div className="relative group w-full sm:flex-1">
              ...
              <Input
                ...
                className="pl-11 h-12 rounded-xl bg-white/40 border-white/60 focus:bg-white focus:ring-2 focus:ring-cyan-100 shadow-sm transition-all text-base w-full"
              />
            </div>
            ...
            <div className="flex gap-3 w-full sm:w-auto">
              <select
                ...
                className="h-12 px-4 rounded-xl border border-gray-200 bg-white/50 hover:bg-blue-50 focus:ring-2 focus:ring-blue-100 outline-none shadow-sm text-gray-700 font-medium flex-1 sm:w-40 transition-colors"
              >
        ...
        <div className="rounded-[24px] backdrop-blur-xl bg-white/60 border border-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden min-h-[400px]">
          {loading ? (
            <div className="flex flex-col items-center justify-center h-[400px] opacity-70">
              ...
              <p className="text-sm font-medium text-gray-600 tracking-wide">Fetching tally records...</p>
            </div>
          ) : viewMode === "summary" ? (
            <>
              {filteredTallies.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-[400px] text-gray-500">
                  <ClipboardList className="w-12 h-12 mb-4 opacity-30 text-cyan-500" />
                  <p className="text-lg font-medium text-gray-900">No tallies found</p>
                        ...
                        {filteredTallies.map((tally) => (
                          <tr key={tally.studentId} className="hover:bg-white/80 transition-colors group">
                            <td className="py-4 px-6">
                              <p className="font-bold text-gray-900 group-hover:text-blue-700 transition-colors">{tally.studentName}</p>
                            ...
                            <td className="py-4 px-6 text-center">
                              <span className="inline-flex items-center justify-center min-w-[2rem] rounded-lg bg-red-50 px-2 py-1 text-sm font-bold text-red-700 border border-red-100">
                                {tally.count}
                              </span>
                            </td>
                            <td className="py-4 px-6 text-center">
                              <span className="inline-flex items-center justify-center min-w-[3rem] rounded-lg bg-emerald-50 px-2 py-1 text-sm font-bold text-emerald-700 border border-emerald-100">
                                ₹{tally.rupees}
                              </span>
                            </td>
                            <td className="py-4 px-6">
                            ...
                            <td className="py-4 px-6 text-right">
                              <Button
                                ...
                                className="gap-1.5 h-9 rounded-xl bg-yellow-500 hover:bg-yellow-600 text-white shadow-sm border border-yellow-400 font-semibold"
                              >
                  ...
                  <div className="md:hidden grid grid-cols-1 gap-3 p-4 bg-gray-50/30">
                    ...
                    {filteredTallies.map((tally) => (
                      <div key={tally.studentId} className="bg-white border border-gray-200/70 rounded-2xl p-4 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] flex flex-col gap-3 relative overflow-hidden group">
                        <div className="absolute top-0 left-0 w-1.5 h-full bg-blue-400 rounded-l-2xl opacity-50"></div>
                        ...
                        <div className="pl-2 flex items-center gap-3">
                          <div className="bg-red-50 border border-red-100 rounded-xl px-3 py-2 flex-1 text-center">
                            <p className="text-[10px] font-bold text-red-500 uppercase tracking-wider mb-0.5">Count</p>
                            <p className="text-lg font-black text-red-700 leading-none">{tally.count}</p>
                          </div>
                          <div className="bg-emerald-50 border border-emerald-100 rounded-xl px-3 py-2 flex-1 text-center">
                            <p className="text-[10px] font-bold text-emerald-500 uppercase tracking-wider mb-0.5">Amount</p>
                            <p className="text-lg font-black text-emerald-700 leading-none">₹{tally.rupees}</p>
                          </div>
                        </div>
                        ...
                        <div className="pl-2 pt-2 border-t border-gray-100 flex items-center justify-between mt-1">
                          ...
                          <Button
                            ...
                            onClick={() => awardStar(tally.studentId, tally.studentName)}
                            disabled={awardingStars === tally.studentId}
                            className="shrink-0 gap-1 h-9 rounded-xl bg-gradient-to-tr from-yellow-500 to-amber-400 hover:from-yellow-600 hover:to-amber-500 text-white shadow-sm border border-yellow-400/50 font-bold px-4"
                          >
                            {awardingStars === tally.studentId ? (
              ...
              {tallies.filter(t => t.tallyType === 'NORMAL').length === 0 ? (
                ...
                <div className="space-y-4 max-h-[600px] overflow-y-auto custom-scrollbar pr-2">
                  ...
                  {tallies
                    ...
                    .map((tally) => (
                      <div key={tally.id} className="bg-white border border-gray-200/80 rounded-2xl p-4 sm:p-5 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.03)] hover:shadow-md hover:border-blue-200 transition-all">
                        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                          ...
                          <div className="flex flex-col justify-center border-t border-gray-100 sm:border-0 pt-3 sm:pt-0">
                            ...
                            <p className="font-bold text-blue-900 bg-blue-50 border border-blue-100 inline-block px-3 py-1.5 rounded-xl text-sm self-start break-words max-w-full">
                              {tally.tallyTypeName}
                            </p>
                          </div>
                          ...
                          <div className="flex flex-col justify-center border-t border-gray-100 sm:border-0 pt-3 sm:pt-0">
                            ...
                            <div className="flex gap-2">
                              ...
                              <span className="inline-flex items-center rounded-lg bg-emerald-50 border border-emerald-100 px-2.5 py-1 text-xs font-bold text-emerald-700">
                                ₹{tally.count * 10}
                              </span>
                            </div>
                          </div>

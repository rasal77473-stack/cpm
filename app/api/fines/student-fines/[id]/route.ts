...
export async function PUT(
  ...
  try {
    ...
    if (!isPaid || (isPaid !== "YES" && isPaid !== "NO")) {
      return NextResponse.json({ error: "isPaid must be 'YES' or 'NO'" }, { status: 400 })
    }

    const [updated] = await db
      .update(studentFines)
      .set({ isPaid, paidDate: isPaid === "YES" ? new Date().toISOString() : null })
...
export async function DELETE(
  ...
  try {
    ...
    await db.delete(studentFines).where(eq(studentFines.id, fineId))
    ...
    console.error("DELETE /api/fines/student-fines/[id] error:", errorMessage)
    return NextResponse.json({ error: errorMessage }, { status: 500 })
  }
}
